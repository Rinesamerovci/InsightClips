...
const pattern = /class ExportSettings\(BaseModel\):/;
if (!pattern.test(content)) throw new Error('Missing ExportSettings class');
const insertion = `class GenerationSettingsInput(BaseModel):
...
`;
content = content.replace(pattern, `${insertion}class ExportSettings(BaseModel):`);
